...
\liststyleWWNumiithm }
  ...
  \item[$\bullet$] when b = }\textit{\textcolor{black}{e}} = 2.7182818 the function is called natural logarithm and denoted ln(x)
\end{description}
          {\textbf{\textcolor{black}{Logarithmic identities}}}\textcolor{black}{:}

   \begin{description}
  \item[$\bullet$] \textcolor{black}{Product: } $\log _b(x\ast y)=\log _bx+\log _by$
 \item[$\bullet$]  \textcolor{black}{Quotient: } $\log _b(x/y)=\log _bx-\log _by$
 \item[$\bullet$]  \textcolor{black}{Power: } $\log _b(x^p)=p\log _bx$
 \item[$\bullet$] \textcolor{black}{Root: } $\log _b\sqrt[p]x=\frac{\log _bx} p$
\end{description}
 \clearpage
